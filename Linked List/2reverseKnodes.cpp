...
//reversing k nodes
struct node* reverseK(struct node* head, int k){        //T.C = O(N)
    //iteration for reversing the first k nodes
    struct node*prev=NULL;
    struct node*curr=head;
    struct node*next;
    int count=0;//counter to check the first k nodes and reverse them
    ...
    //the above function reverses for the first k nodes in the linked list and now we have to call for the rest of the linked list
    if(next!=NULL){     //we have to check if our next ptr is not null
    head->next=reverseK(next, k);
    }
    return prev; //as prev ptr is our new head;
}
    ...
    while(n!=6){
        ...
        switch(n){
            ...
            case 4:
                ...
                break;
            case 5:
                cout<<"reverse k nodes linked list for k: ";
                cin>>data;
                // head=reverseRecursive(head);
                head=reverseK(head, data);
                break;
        }
    }
    return 0;
